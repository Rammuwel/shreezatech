    ...
    <div class="container relative mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mx-auto mb-10 max-w-3xl text-center">
            ...
            <p class="mt-5 text-lg leading-8 text-muted">
                Here are answers to the most common questions about our {{ $category['name'] }} expertise.
            </p>
        </div>
        <div class="mx-auto w-full space-y-6">
            @foreach ($category['faqs'] as $faq)
            <div x-data="{ open: false }" class="rounded-3xl border border-border bg-card overflow-hidden">
                <button @click="open = !open" class="flex w-full items-center justify-between p-6 text-left">
                    <h3 class="text-lg font-semibold text-heading">{{ $faq['question'] }}</h3>
                    <i class="fa-solid fa-plus text-primary transition duration-300" :class="{ 'rotate-45': open }"></i>
                </button>
                <div x-show="open" x-collapse>
                    <div class="border-t border-border px-6 py-5">
                        <p class="leading-8 text-muted">{{ $faq['answer'] }}</p>
                    </div>
                </div>
            </div>
            @endforeach
