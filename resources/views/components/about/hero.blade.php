<section class="portfolio-hero overflow-hidden  bg-background">
    <div class="relative py-5 mt-20  flex items-center">

        <!-- ================= Background ================= -->

        <div class="absolute inset-0">

            <!-- Grid -->

            <div
                class="absolute inset-0 opacity-10
            bg-[linear-gradient(to_right,#1f2937_1px,transparent_1px),linear-gradient(to_bottom,#1f2937_1px,transparent_1px)]
            bg-size-[80px_80px]">
            </div>

            <!-- Blue Glow -->
            <div
                class="absolute  top-20 left-0 w-96 h-96  bg-linear-to-r from-primary/10 to-transparent  rounded-full blur-[140px]">
            </div>

            <!-- Gold Glow -->

            <div
                class="absolute bottom-0 right-0 w-125 h-125  bg-linear-to-r to-transparent from-background  to-b rounded-full blur-[680px]">
            </div>


        </div>

        <!-- ================= Container ================= -->

        <div
            class="relative z-10 max-w-7xl mx-auto px-6 lg:px-8">

            <div
                class="grid lg:grid-cols-4 gap-20 items-center">

                <!-- ================================================= -->
                <!-- LEFT -->
                <!-- ================================================= -->

                <div class="col-span-2">

                    <!-- Badge -->

                    <div
                        class="portfolio-hero-badge  inline-flex items-center gap-3  px-3 py-1">



                        <small
                            class="text-secondary ">

                            About Shreeza

                        </small>
                    </div>

                    <!-- Heading -->

                    <h1
                        class="portfolio-hero-title mt-3 text-3xl lg:text-5xl font-semibold leading-tight text-heading">

                        Empowering Businesses
                        
                        <br>
                        <span
                            class="bg-linear-to-r from-primary via-blue-400 to-secondary bg-clip-text text-transparent">
                            Through Technology

                        </span>

                    </h1>

                    <!-- Description -->

                    <p
                        class="portfolio-hero-description mt-8 max-w-xl text-lg leading-8 text-text">

                        Shreeza is a technology consulting and software development company dedicated to building innovative digital solutions that help businesses grow, transform, and succeed. We combine strategy, creativity, and cutting-edge technology to deliver secure, scalable, and future-ready software for organizations worldwide.

                    </p>

                    <!-- Buttons -->

                    <div
                        class="portfolio-hero-buttons mt-10 flex flex-wrap gap-5">

                        <!-- <a
                            href="#"
                            class="rounded-full bg-primary px-8 py-4 font-semibold text-white transition hover:scale-105">

                            Start Your Project

                        </a>

                        <a
                            href="#"
                            class="rounded-full border border-border px-8 py-4 font-semibold text-heading hover:border-primary">

                            Explore Work

                        </a> -->

                    </div>
                </div>

                <!-- ================================================= -->
                <!-- RIGHT -->
                <!-- ================================================= -->

                <div class="relative flex justify-center col-span-2">

                    <!-- Background Glow -->

                    <!-- Image -->
                    <img
                        src="{{ asset('/images/about/shreeza-tech-about-hero.webp') }}"
                        class="relative  w-207 hero-float"
                        alt="Shreeza Tech about us hero banner">

                    <!-- Bottom Fade -->
                    <div class="absolute inset-0 bg-linear-to-t from-background via-transparent to-transparent z-20"></div>
                    <div class="absolute inset-0 bg-linear-to-b from-background via-transparent to-transparent z-20"></div>
                    <div class="absolute inset-0 bg-linear-to-r from-background via-transparent to-transparent z-20"></div>
                    <div class="absolute inset-0 bg-linear-to-l from-background via-transparent to-transparent z-20"></div>

                </div>
            </div>

        </div>
    </div>

    <div class="max-w-7xl mx-auto mt-4  border-border bg-card rounded-2xl mb-3 ">

        <!-- Features -->
        <div class="grid gap-8 md:grid-cols-3 p-3">

            <!-- Card -->
            <div class="group flex flex-col items-center justify-center  transition-all duration-500 md:border-r border-border">

                <div class="flex h-16 w-16 items-center justify-center rounded-2xl bg-secondary/10 text-secondary text-2xl">

                    <i class="fa-solid fa-bullseye"></i>

                </div>

                <h3 class="mt-2 text-xl font-semibold text-heading">

                    Our Mission

                </h3>

                <p class=" text-muted leading-7 text-center justify-center">

                    Empowering businesses through technology and innovation.

                </p>

            </div>

            <!-- Card -->
            <div class="group flex flex-col items-center justify-center  transition-all duration-500 md:border-r border-border">

                <div class="flex h-16 w-16 items-center justify-center rounded-2xl bg-secondary/10 text-secondary text-2xl">

                    <i class="fa-solid fa-eye"></i>

                </div>

                <h3 class="mt-2 text-xl font-semibold text-heading">

                    Our Vision

                </h3>

                <p class=" text-muted leading-7 text-center justify-center">

                    To be a global leader in delivering digital excellence.

                </p>

            </div>

            <!-- Card -->
            <div class="group flex flex-col items-center justify-center   transition-all duration-500 ">

                <div class="flex h-16 w-16 items-center justify-center rounded-2xl bg-secondary/10 text-secondary text-2xl">

                    <i class="fa-solid fa-gem"></i>

                </div>

                <h3 class="mt-2 text-xl font-semibold text-heading">

                    Our Values

                </h3>

                <p class=" text-muted leading-7 text-center justify-center">

                    Integrity, innovation, quality, and customer success.

                </p>

            </div>

        </div>

    </div>
</section>